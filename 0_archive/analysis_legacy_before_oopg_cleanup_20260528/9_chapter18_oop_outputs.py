"""
Create Chapter 18-style OOP budget-share figure and audit outputs.

Outputs are stored in:
    6_output/main_output/chapter18/oop/
"""
...
OUT = PROJECT_ROOT / "6_output" / "main_output" / "chapter18" / "oop"
DATA_PATH = PROJECT_ROOT / "1_data" / "2_clean" / "catastrophic_health_exp.dta"
BOX1_CSV = OUT / "oop_box18_1_incidence_intensity.csv"
BOX2_CSV = OUT / "oop_box18_2_distribution_sensitive.csv"
...
def box1_table_data(box1: pd.DataFrame) -> list[list[str]]:
    rows: list[list[str]] = []
    rows.append(["OOP as share of total expenditure", "", "", "", "", ""])
    for metric, col in [
...
def make_budget_share_curve(df: pd.DataFrame) -> None:
    ...
    x_tot, y_tot = weighted_curve(df, "oop_sh_tot")
    x_nf, y_nf = weighted_curve(df, "oop_sh_nf")
    pd.DataFrame(
        {
            "cum_hh_total": pd.Series(x_tot),
            "oop_share_total": pd.Series(y_tot),
            "cum_hh_ctp": pd.Series(x_nf),
            "oop_share_ctp": pd.Series(y_nf),
        }
    ).to_csv(OUT / "oop_budget_share_curve_data.csv", index=False)
    ...
    ax.set_ylabel("OOP budget share", fontsize=9)
    ...
    ax.grid(False)
    ax.text(0.19, 0.30, "OOP/nonfood exp.", fontsize=9.2, color="#222222")
    ax.text(0.05, 0.05, "OOP/total exp.", fontsize=9.2, color="#222222")
    ax.set_title(
        "OOP Total and Nonfood Budget Share against Cumulative Percentage\n"
    ...
    fig.savefig(OUT / "oop_budget_share_curve.png", dpi=240, bbox_inches="tight", facecolor=BG)
    fig.savefig(OUT / "oop_budget_share_curve.pdf", bbox_inches="tight", facecolor=BG)
    plt.close(fig)
...
def make_box1(box1: pd.DataFrame, df: pd.DataFrame) -> None:
    ...
    ax_table.text(
        0.0,
        0.995,
        "Incidence and Intensity of Catastrophic OOP Payments, Nepal NLSS IV 2022/23\n"
    ...
    table = ax_table.table(
        ...
        colLabels=["Catastrophic payment measure", "5%", "10%", "15%", "25%", "40%"],
        loc="upper left",
        cellLoc="right",
    ...
    ax.spines[["bottom", "left"]].set_color("#394535")
    ax.grid(False)
    ax.text(0.19, 0.30, "OOP/capacity-to-pay exp.", fontsize=9.2, color="#222222")
    ax.text(0.05, 0.05, "OOP/total exp.", fontsize=9.2, color="#222222")
    ax.set_title(
        "OOP Total and Capacity-to-Pay Budget Share against Cumulative Percentage\n"
        "of Households Ranked by Decreasing Budget Share, Nepal NLSS IV 2022/23",
        fontsize=9.5,
        fontstyle="italic",
        loc="left",
    ...
    fig.savefig(OUT / "oop_box18_1_incidence_intensity.png", dpi=240, bbox_inches="tight", facecolor=BG)
    fig.savefig(OUT / "oop_box18_1_incidence_intensity.pdf", bbox_inches="tight", facecolor=BG)
    plt.close(fig)
...
def make_box2(box2: pd.DataFrame) -> None:
    ...
    ax.text(
        0.0,
        1.03,
        "Box 18.2-style Distribution-Sensitive OOP Measures",
    ...
    table = ax.table(
        ...
        colLabels=["Distribution-sensitive measure", "5%", "10%", "15%", "25%", "40%"],
        loc="upper left",
        cellLoc="right",
    ...
    fig.savefig(OUT / "oop_box18_2_distribution_sensitive.png", dpi=240, bbox_inches="tight", facecolor=BG)
    fig.savefig(OUT / "oop_box18_2_distribution_sensitive.pdf", bbox_inches="tight", facecolor=BG)
    plt.close(fig)


def write_excel_and_summary(box1: pd.DataFrame, box2: pd.DataFrame) -> None:
    with pd.ExcelWriter(OUT / "oop_chapter18_results.xlsx", engine="openpyxl") as writer:
    ...
    md = f"""# Chapter 18 OOP Results

Scope: OOP, defined as OOPG plus NCD OOP converted to monthly terms.
...
- OOP > 10% of total expenditure: {pct(float(total10['headcount']))}; overshoot: {pct(float(total10['overshoot']))}; MPO: {pct(float(total10['mpo']))}.
- OOP > 40% of nonfood expenditure: {pct(float(nf40['headcount']))}; overshoot: {pct(float(nf40['overshoot']))}; MPO: {pct(float(nf40['mpo']))}.
- Concentration index for OOP > 10% of total expenditure: {num(float(total10_ci['concentration_headcount']))}; rank-weighted headcount: {pct(float(total10_ci['rank_weighted_headcount']))}.
- Concentration index for OOP > 40% of nonfood expenditure: {num(float(nf40_ci['concentration_headcount']))}; rank-weighted headcount: {pct(float(nf40_ci['rank_weighted_headcount']))}.
"""
    (OUT / "oop_chapter18_results.md").write_text(md, encoding="utf-8")
...
def main() -> None:
    ...
    print(f"Saved Chapter 18 OOP outputs to {OUT}")
