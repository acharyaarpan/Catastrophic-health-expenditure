*------------------------------------------------------------------------------*
*           Chapter 18 OOPG: Catastrophic Payments at Book Thresholds           *
/*

    Purpose:
        Produce Chapter 18-style incidence/intensity and distribution-sensitive
        catastrophic payment measures for OOPG only.
    ...
    Notes:
        - OOPG is communicable/injury OOP from NLSS Section 8(B).
        ...
        - Distribution-sensitive measures rank by pre_oopg.
*/
*------------------------------------------------------------------------------*
...
global out "`root'/6_output/main_output/chapter18/oopg"

cap mkdir "`root'/6_output/main_output/chapter18"
cap mkdir "$out"

cap log close
log using "$out/chapter18_oopg.log", replace
...
foreach denom in total nonfood {
    ...
    foreach z of local thresholds {
        ...
        label variable ch18_oopg_`stub'`z' "Chapter 18 OOPG CHE: `z'% `denom'"
        label variable ch18_over_oopg_`stub'`z' "Chapter 18 OOPG overshoot: `z'% `denom'"
    }
}
...
preserve
    use `box1data', clear
    export delimited using "$out/oopg_box18_1_incidence_intensity.csv", replace
restore
...
preserve
    use `box2data', clear
    export delimited using "$out/oopg_box18_2_distribution_sensitive.csv", replace
restore
