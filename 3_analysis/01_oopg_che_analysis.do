*------------------------------------------------------------------------------*
*        OOPG catastrophic health expenditure analysis - active workflow        *
/*

    Purpose:
        Produce the active OOPG-only analysis outputs for the manuscript.
...
if "$workspace" == "" {
    ...
    else if fileexists("D:/Projects/CIH-project/consumption/0_master.do") {
        global workspace "D:/Projects/CIH-project/consumption"
    }
    else {
...
cap drop food_nom_mo x_oopg_mo food_share_oopg food_equiv_mo food_mid_4555
...
cap drop post_welfare poor_post gap_post ngap_post poor_pre_oopg gap_pre_oopg
cap drop ngap_pre_oopg diff_h_oopg diff_gap_oopg diff_ngap_oopg pushed_oopg

gen double post_welfare = pcep
gen byte poor_post = (post_welfare < pline) if !missing(post_welfare, pline)
gen double gap_post = poor_post * (pline - post_welfare)
gen double ngap_post = gap_post / pline

gen byte poor_pre_oopg = (pre_oopg < pline) if !missing(pre_oopg, pline)
gen double gap_pre_oopg = poor_pre_oopg * (pline - pre_oopg)
gen double ngap_pre_oopg = gap_pre_oopg / pline
...
gen byte pushed_oopg = (pre_oopg >= pline & post_welfare < pline) ///
    if !missing(pre_oopg, post_welfare, pline)
...
post `params' ("food_share_p45") (`p45') ("weighted 45th percentile of OOPG-inclusive food share")
post `params' ("food_share_p55") (`p55') ("weighted 55th percentile of OOPG-inclusive food share")
...
di as result "Saved active OOPG manuscript tables to $tables"
